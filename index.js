...
app.get('/update/:cafeid', isAuthorized, async (req, res) => {
  const orders = await Order.find({ seen: 'false', cafe_id: req.params.cafeid });

  if (orders.length == 0) return sendResponse(res, 200, 'no_updates', null, null);
  ...
  return sendResponse(res, 200, 'new_orders', payload, null);
})
...
app.put('/order/:cafeid', isAuthorized, async (req, res) => {
  await Order.updateOne(
    { order_id: req.body.order.order_id, cafe_id: req.params.cafeid },
    { $set: { ...req.body.order } }
  )

  return sendResponse(res, 200, 'order_updated', {orderId: req.body.order.order_id}, null)
})
...
app.get('/menu/:cafeid', async (req, res) => {
  ...
  for (let i = 0; i < cats.length; i++) {
    ...
    for (let j = 0; j < cat.children.length; j++) {
      item = await Item.find({ cat: cat.cat_id, cafe_id, item_id: cat.children[j]?.item_id ?? cat.children[j] });
      items.push(item[0]);
    }
